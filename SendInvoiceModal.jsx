...
import { sendInvoiceSentEmail } from '../emailUtils'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: jobData } = await supabase
        .from('jobs')
        .select('posted_by, profiles:posted_by (email, full_name)')
        .eq('id', job?.id)
        .single()

      if (jobData?.profiles) {
        sendInvoiceSentEmail({
          clientId: jobData.posted_by,
          clientEmail: jobData.profiles.email,
          clientName: jobData.profiles.full_name,
          jobTitle: job?.title,
          amount: clientPays > 0 ? clientPays.toFixed(2) : null,
          notes: notes || null
        }).catch(err => console.warn('Email failed (non-fatal):', err))
      }
