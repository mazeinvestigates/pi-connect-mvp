  ...
  try {
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        console.log('userId:', userId, 'tier:', tier, 'interval:', interval)
        if (!userId || !tier) {
          console.error('Missing userId or tier in metadata — skipping')
          break
        }

        const subscription = await stripe.subscriptions.retrieve(session.subscription)
        console.log('Subscription status:', subscription.status)
        console.log('Updating pi_profiles for userId:', userId)
