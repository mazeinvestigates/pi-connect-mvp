    ...
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        const { data: referral } = await supabase
          ...
          .select('*, referring_pi:referring_pi_id (stripe_customer_id)')
        ...
        if (referral?.referring_pi?.stripe_customer_id) {
          try {
            ...
            await stripe.customers.createBalanceTransaction(
              ...
              {
                amount: -referral.credit_amount_cents, // negative = credit
                currency: 'usd',
            ...
            await supabase.from('notifications').insert({
              ...
              message: `${referral.referred_pi_email} subscribed using your referral. A $${(referral.credit_amount_cents / 100).toFixed(2)} credit has been applied to your account.`,
              related_type: 'subscription'
            })
          } catch (err) {
