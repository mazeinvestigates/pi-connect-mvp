...
export default function ProfileEditPage({ user, profile, onNavigate, onProfileUpdate }) {
  // Preserve scroll position when switching browser tabs
  React.useEffect(() => {
    const handleVisibility = () => {
      if (document.visibilityState === 'visible') {
        const saved = sessionStorage.getItem('profileedit_scroll')
        if (saved) window.scrollTo(0, parseInt(saved, 10))
      } else {
        sessionStorage.setItem('profileedit_scroll', String(window.scrollY))
      }
    }
    document.addEventListener('visibilitychange', handleVisibility)
    return () => {
      document.removeEventListener('visibilitychange', handleVisibility)
      sessionStorage.removeItem('profileedit_scroll')
    }
  }, [])
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              <div className="form-group">
                ...
                <p style={{ fontSize: '12px', color: '#6b7280', marginBottom: '8px' }}>
                  How far will you travel for a job? Controls which searches you appear in.
                </p>
              ...
              <div style={{ padding: '8px 0' }}>
                ...
                <p style={{ fontSize: '12px', color: '#374151', marginTop: '6px' }}>
                  {formData.notificationRadius === -1
                    ? 'Nationwide'
                    : `${formData.notificationRadius} miles`}
                </p>
              </div>
            </div>
