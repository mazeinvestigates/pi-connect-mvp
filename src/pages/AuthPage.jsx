...
export default function AuthPage({ onSuccess, onNavigate }) {
  ...
  const handlePISignup = async (e) => {
    ...
    try {
      ...
      console.log('Referral check — capturedRef:', capturedRef, 'userId:', authData?.user?.id)
      console.log('All localStorage keys:', Object.keys(localStorage))
      if (capturedRef && authData?.user?.id) {
        try {
          console.log('Calling recordSignupReferral with:', capturedRef)
        ...
        console.log('No ref code found — skipping referral recording')
      }
    } catch (err) {
      setError(err.message)
    } finally {
