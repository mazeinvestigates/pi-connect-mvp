...
export default function AuthPage({ onSuccess }) {
  const [mode, setMode] = useState('login') // 'login' | 'signup' | 'pi-signup' | 'forgot-password' | 'reset-password'
  ...
  const handlePISignup = async (e) => {
    ...
    try {
      ...
      if (authData.user) {
        // Sign in immediately to establish session so RLS allows the insert
        ...
        // Now insert pi_profiles with an active session
      ...
      setMessage('PI account created! Please check your email to confirm before signing in.')
    } catch (err) {
      setError(err.message)
    } finally {
      setLoading(false)
  ...
  // Detect reset password redirect from Supabase email link
  React.useEffect(() => {
        ...
        {mode === 'login' && (
          <form onSubmit={handleLogin} className="auth-form">
            ...
            <div className="auth-links">
              ...
              <button type="button" className="link-button" onClick={() => { setMode('signup'); setError(null); setMessage(null) }}>
                Need an account? Sign up
              </button>
              <button type="button" className="link-button" onClick={() => { setMode('pi-signup'); setError(null); setMessage(null) }}>
