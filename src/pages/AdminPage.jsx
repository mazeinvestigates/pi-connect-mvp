...
export default function AdminPage({ user, profile, onNavigate }) {
  ...
  const handleVerifyPI = async (piId, verified) => {
    ...
    try {
      ...
      // Get PI profile data before updating (need doc paths for deletion)
      ...
      // On approval: clear document paths from DB (will delete from storage below)
      if (verified) {
        updateData.license_document_path = null
        updateData.gov_id_document_path = null
        updateData.selfie_document_path = null
        updateData.license_submitted_at = null
        updateData.gov_id_submitted_at = null
      }
      ...
      // On approval: delete verification documents from storage
      if (verified && pi) {
        const pathsToDelete = [
        ...
        if (pathsToDelete.length > 0) {
          await supabase.storage.from('pi-documents').remove(pathsToDelete)
        }
      }
      ...
      // TODO: re-enable email notification once Edge Function 401 is resolved
      // if (pi?.email) { ... }
            ...
            <div className="admin-content">
              ...
              <div className="admin-table">
                <table>
                  ...
                  <tbody>
                    {allUsers.map(u => {
                      // pi_profiles can come back as object or array depending on relationship type
                      const piProfile = Array.isArray(u.pi_profiles)
                        ? u.pi_profiles[0]
                        : u.pi_profiles
                      const isPI = !!piProfile
                      return (
                        <tr key={u.user_id}>
