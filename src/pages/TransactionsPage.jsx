...
export default function TransactionsPage({ user, profile, onNavigate }) {
  ...
  const loadTransactions = async () => {
    ...
    try {
      let query = supabase
        .from('transactions')
        .select('*, job:jobs!transactions_job_fkey (title, city, state, investigation_type)')
      ...
      // Enrich with client names via separate query
      if (data && data.length > 0) {
        ...
        const enriched = data.map(t => ({
          ...t,
          client_profile: profileMap[t.client_id] || null
        }))
