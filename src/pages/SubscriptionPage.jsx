...
export default function SubscriptionPage({ user, profile, onNavigate }) {
  ...
  useEffect(() => {
    if (user?.id) {
      fetch('/api/get-customer-balance', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: user.id })
      })
        .then(r => r.json())
        .then(d => { if (d.balanceCents > 0) setCreditBalance(d.balance) })
        .catch(() => {})
    }
  }, [user?.id])
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 24px' }}>
      ...
      <div style={{ marginTop: '32px', padding: '16px', background: '#f9fafb', borderRadius: '8px' }}>
        {creditBalance && (
          <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '10px', padding: '12px 18px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontSize: '24px' }}>🎁</span>
            <div>
              <p style={{ fontWeight: '600', fontSize: '14px', margin: '0 0 2px', color: '#166534' }}>Account Credit: ${creditBalance}</p>
              <p style={{ fontSize: '12px', color: '#4ade80', margin: 0, color: '#15803d' }}>This credit will be automatically applied to your next invoice.</p>
            </div>
          </div>
        )}
