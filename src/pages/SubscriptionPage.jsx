...
const TIERS = [
  ...
  },
  {
    ...
    features: [
      'Everything in Basic',
      '+5 point boost in search match score',
      'Gold "Featured" badge on profile',
      'Priority placement in search results',
      'Enhanced visibility to clients',
    ]
  },
  {
    key: 'premium',
    ...
    features: [
      ...
      '+10 point boost in search match score',
      'Indigo "Premium" badge on profile',
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 24px' }}>
            ...
            <div key={tier.key} style={{ border: tier.popular ? '2px solid #667eea' : '1px solid #e5e7eb', borderRadius: '12px', padding: '24px', position: 'relative', background: 'white' }}>
              ...
              {isCurrentTier && isSubscribed ? (
                ...
                <button onClick={() => handleSubscribe(tier.key)} disabled={!!loading}
                  ...
                  {loading === tier.key ? 'Redirecting to Stripe...' : isSubscribed ? 'Switch to this plan' : `Get ${tier.name}`}
                </button>
              )}
            </div>
          )
      ...
      <div style={{ marginTop: '32px', padding: '16px', background: '#f9fafb', borderRadius: '8px' }}>
        <p style={{ fontSize: '12px', color: '#9ca3af', margin: 0, textAlign: 'center' }}>
          All plans include full platform access. Membership fees are separate from the 10% platform fee on job transactions. Cancel anytime — you keep your benefits until the end of your billing period. Powered by Stripe.
        </p>
      </div>
    </div>
  )
