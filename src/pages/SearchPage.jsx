...
import { calculateMatchScore, getMatchColor, sortByMatch } from '../matchingAlgorithm'
...
export default function SearchPage({ user, onNavigate }) {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      // Create a mock job object for matching
      const mockJob = {
        location: searchData.city && searchData.state 
          ? `${searchData.city}, ${searchData.state}` 
          : searchData.state || 'United States',
        investigation_type: searchData.specialties[0] || 'General Investigation',
        budget_min: 500,
        budget_max: 2000
      }

      // Calculate match scores and sort by AI matching
      results = sortByMatch(results, mockJob)
