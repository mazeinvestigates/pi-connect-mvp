...
export default function SearchPage({ user, onNavigate }) {
  const [view, setView] = useState(() => {
    return sessionStorage.getItem('search_view') || 'search'
  })
  ...
  const [piMatches, setPiMatches] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem('search_piMatches') || '[]') } catch { return [] }
  })
  const [premiumResults, setPremiumResults] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem('search_premiumResults') || '[]') } catch { return [] }
  })
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const premium = sortedResults.premium || []
      const all = sortedResults.all || []
      setPremiumResults(premium)
      setPiMatches(all)
      try {
        sessionStorage.setItem('search_premiumResults', JSON.stringify(premium))
        sessionStorage.setItem('search_piMatches', JSON.stringify(all))
        sessionStorage.setItem('search_view', 'results')
      } catch {}
      setView('results')
    } catch (error) {
  ...
  return (
    <div className="results-page">
      <div className="results-header">
        <button onClick={() => {
          setView('search')
          sessionStorage.removeItem('search_view')
          sessionStorage.removeItem('search_piMatches')
          sessionStorage.removeItem('search_premiumResults')
