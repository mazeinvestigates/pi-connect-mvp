              ...
              {showArchive && (
                <div style={{ marginTop: '12px' }}>
                  {jobs.filter(a => ['completed','cancelled'].includes(a.status)).map(app => {
                    ...
                    const badge = getStatusBadge(app.status)
                    return (
                      <div key={app.id} className="job-management-card" style={{ opacity: 0.85 }}>
                        <div className="job-card-header">
                          <div>
                            <h3>{job?.title}</h3>
                            <p className="job-meta">📍 {job?.location} · {job?.investigation_type}</p>
                        ...
                        {app.status === 'completed' && app.agreed_amount_cents && (
                          <p style={{ fontSize: '13px', color: '#059669', padding: '8px 16px', margin: 0 }}>
                            💰 Escrow amount: ${(app.agreed_amount_cents / 100).toFixed(2)} — payout pending Stripe Connect
                          </p>
                        )}
                      </div>
                    )
