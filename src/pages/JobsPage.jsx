  ...
  return (
    <div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                              ...
                              {app.payment_mode === 'escrow' && app.agreed_amount_cents && (
                                <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '10px 14px' }}>
                                  ...
                                  <p style={{ fontSize: '13px', margin: 0 }}>
                                    ${(app.agreed_amount_cents / 100).toFixed(2)} — {app.payment_status === 'paid' ? 'funded by client' : app.payment_status}
                                  </p>
                                </div>
                              )}
                              {app.payment_mode === 'invoice' && app.payment_status === 'paid' && (
                                <div style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "10px 14px", marginBottom: "4px" }}>
                                  <p style={{ fontWeight: '600', fontSize: '13px', marginBottom: '2px' }}>💰 Invoice Payment</p>
                                  <p style={{ fontSize: '13px', margin: 0 }}>
                                    {app.invoice_amount_cents ? `$${(app.invoice_amount_cents / 100).toFixed(2)} — paid` : 'Paid'}
                                  </p>
                                </div>
                              )}
                              ...
                              {app.contract_status && (
                                <div style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "10px 14px", marginBottom: "4px" }}>
                                  ...
                                  <p style={{ fontSize: '13px', margin: '0 0 6px' }}>
                                    {app.contract_type === 'platform' ? 'PI Connect standard agreement' : 'Custom contract uploaded'} —{' '}
                                    {app.contract_status === 'signed'
                                      ? `Signed by client${app.contract_signed_at ? ` on ${new Date(app.contract_signed_at).toLocaleDateString()}` : ''}`
                                      : app.contract_status === 'declined' ? 'Declined by client'
                                      : 'Pending signature'}
                                  </p>
                                  {app.contract_file_path && (
                                    <button style={{ background: 'none', border: 'none', color: '#667eea', cursor: 'pointer', fontSize: '13px', padding: 0 }}
                                      onClick={async () => {
                                        const { data } = await supabase.storage.from('contracts').createSignedUrl(app.contract_file_path, 3600)
                                        if (data?.signedUrl) window.open(data.signedUrl, '_blank')
                                        else alert('Could not load contract file.')
                                      }}>
                                      📄 Download Contract
                                    </button>
                                  )}
                                  {app.contract_type === 'platform' && app.contract_status === 'signed' && (
                                    <button style={{ background: 'none', border: 'none', color: '#667eea', cursor: 'pointer', fontSize: '13px', padding: 0, display: 'block', marginTop: '4px' }}
                                      onClick={() => {
                                        ...
                                        win.document.write(`<html><body>${generateContractHTML({
                                          piName: profile ? `${profile.first_name || ''} ${profile.last_name || ''}`.trim() : 'PI',
                                          clientName: app.job?.profiles?.full_name || 'Client',
                                          ...
                                          today: new Date(app.contract_signed_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })
                                        })}</body></html>`)
                                        win.document.close()
                                      }}>
                              ...
                              {app.invoice_sent_at && (
                                <div style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "10px 14px", marginBottom: "4px" }}>
                                  ...
                                  <p style={{ fontSize: '13px', margin: '0 0 6px' }}>
                                    Sent {new Date(app.invoice_sent_at).toLocaleDateString()}
                                    {app.invoice_amount_cents ? ` · $${(app.invoice_amount_cents / 100).toFixed(2)}` : ''}
                                    {app.invoice_notes ? ` · "${app.invoice_notes}"` : ''}
                                  </p>
                                  {app.invoice_file_path && (
                                    <button style={{ background: 'none', border: 'none', color: '#667eea', cursor: 'pointer', fontSize: '13px', padding: 0 }}
                                      onClick={async () => {
                                        const { data } = await supabase.storage.from('invoices').createSignedUrl(app.invoice_file_path, 3600)
                                        if (data?.signedUrl) window.open(data.signedUrl, '_blank')
                                        else alert('Could not load invoice file.')
                                      }}>
                                      📄 Download Invoice
                                    </button>
                                  )}
                              ...
                              {app.deliverables_uploaded_at && (
                                <div style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "10px 14px", marginBottom: "4px" }}>
                                  ...
                                  <p style={{ fontSize: '13px', margin: 0 }}>
                                    Uploaded {new Date(app.deliverables_uploaded_at).toLocaleDateString()}
                                  </p>
                              ...
                              {app.status === 'cancelled' && app.cancel_reason && (
                                <div style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "10px 14px", marginBottom: "4px" }} style={{ background: '#fef2f2', border: '1px solid #fecaca' }}>
                                  <p style={{ fontWeight: '600', fontSize: '13px', marginBottom: '2px' }}>✗ Cancellation</p>
                                  <p style={{ fontSize: '13px', margin: 0 }}>Reason: {app.cancel_reason}</p>
                                </div>
                              )}
                            </div>
                          </div>
      ...
      {showInvoiceModal && selectedApp && (
        <SendInvoiceModal
          app={selectedApp}
          job={jobs.find(j => j.id === selectedApp.id)?.job}
          user={user}
                        ...
                        {isExpanded && (
                          <div className="job-card-body">
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                              ...
                              {app.contract_status && (
                                <div className="info-box">
                                  <p style={{ fontWeight: '600', fontSize: '13px', marginBottom: '2px' }}>📄 Contract</p>
                                  <p style={{ fontSize: '13px', margin: 0 }}>
                                    {app.contract_type === 'platform' ? 'PI Connect standard agreement' : 'Custom contract uploaded'} —{' '}
                                    {app.contract_status === 'signed'
                                      ? `Signed by client${app.contract_signed_at ? ` on ${new Date(app.contract_signed_at).toLocaleDateString()}` : ''}`
                              ...
                              {app.invoice_sent_at && (
                                <div className="info-box">
                                  <p style={{ fontWeight: '600', fontSize: '13px', marginBottom: '2px' }}>🧾 Invoice</p>
                                  <p style={{ fontSize: '13px', margin: 0 }}>
                                    Sent {new Date(app.invoice_sent_at).toLocaleDateString()}
                                    {app.invoice_amount_cents ? ` · $${(app.invoice_amount_cents / 100).toFixed(2)}` : ''}
                                    {app.invoice_notes ? ` · "${app.invoice_notes}"` : ''}
