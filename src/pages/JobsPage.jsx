  ...
  return (
    <div>
        ...
        <div className="jobs-grid">
          {jobs.map(job => (
            <JobCard key={job.id} job={job}
              ...
              previousAppStatus={myApplications[job.id]}
      ...
      {showApplicationModal && selectedJob && (
        <JobApplicationModal job={selectedJob} user={user} piProfile={profile} hasApplied={!!myApplications[selectedJob?.id]} previousAppStatus={myApplications[selectedJob?.id]}
  ...
  const loadMyJobs = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .in('status', ['pending', 'accepted', 'in_progress', 'completed', 'cancelled', 'declined'])
  ...
  return (
    <div>
      ...
      {jobs.length === 0 ? (
        ...
        <div>
          {jobs.filter(a => !['completed','cancelled','declined','pending'].includes(a.status)).length === 0 
            && jobs.filter(a => a.status === 'pending').length === 0
            && jobs.length > 0 && (
          ...
          {jobs.filter(a => a.status === 'pending').length > 0 && (
            <div style={{ marginBottom: '24px' }}>
              <h3 style={{ fontSize: '15px', fontWeight: '600', marginBottom: '12px', color: '#92400e', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ⏳ Pending Applications ({jobs.filter(a => a.status === 'pending').length})
              </h3>
              {jobs.filter(a => a.status === 'pending').map(app => {
                const job = app.job
                return (
                  <div key={app.id} style={{ background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: '10px', padding: '14px 16px', marginBottom: '10px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '8px' }}>
                      <div>
                        <h3 style={{ fontSize: '15px', fontWeight: '600', margin: '0 0 4px' }}>{job?.title}</h3>
                        <p style={{ fontSize: '13px', color: '#6b7280', margin: '0 0 2px' }}>📍 {job?.location} · {job?.investigation_type}</p>
                        <p style={{ fontSize: '13px', color: '#6b7280', margin: 0 }}>Client: {job?.profiles?.full_name}</p>
                        {app.proposed_rate && (
                          <p style={{ fontSize: '13px', color: '#6b7280', margin: '2px 0 0' }}>
                            Your rate: ${app.proposed_rate}{app.rate_type === 'flat' ? ' flat fee' : '/hr'}
                          </p>
                        )}
                      </div>
                      <span style={{ fontSize: '12px', background: '#fef3c7', color: '#92400e', border: '1px solid #fcd34d', borderRadius: '6px', padding: '3px 10px', fontWeight: '600', whiteSpace: 'nowrap' }}>
                        ⏳ Awaiting client decision
                      </span>
                    </div>
                    {app.cover_letter && (
                      <p style={{ fontSize: '13px', color: '#374151', marginTop: '10px', borderTop: '1px solid #fde68a', paddingTop: '8px', lineHeight: '1.5' }}>
                        {app.cover_letter.length > 200 ? app.cover_letter.slice(0, 200) + '…' : app.cover_letter}
                      </p>
                    )}
                    <div style={{ marginTop: '10px', borderTop: '1px solid #fde68a', paddingTop: '8px' }}>
                      <button
          ...
          <h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '12px', marginTop: jobs.filter(a => a.status === 'pending').length > 0 ? '8px' : '0', color: '#374151' }}>
            ✓ Accepted Jobs
          </h3>
          {jobs.filter(a => !['completed','cancelled','declined','pending'].includes(a.status)).map(app => {
            const job = app.job
                ...
                {app.payment_status === 'paid' && (
                  <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '8px', padding: '10px 14px', marginBottom: '12px' }}>
                    <p style={{ fontWeight: '600', fontSize: '14px', color: '#166534', margin: 0 }}>
                      ✓ {app.payment_mode === 'escrow' ? 'Retainer paid — you can begin work' : 'Payment received'}
                    </p>
                      ...
                      <div style={{ marginTop: '10px' }}>
                        ...
                        {app.escrow_amount_cents && (
                          <p style={{ fontSize: '12px', color: '#059669', marginTop: '4px' }}>
                            ✓ Retainer amount set: ${(app.escrow_amount_cents / 100).toFixed(2)} — client will be prompted to pay this amount before work begins.
                          </p>
                        )}
                        {!app.escrow_amount_cents && (
                          <p style={{ fontSize: '12px', color: '#d97706', marginTop: '4px' }}>
                            ⚠️ Set the retainer amount before the client can pay.
                          </p>
                        )}
                      </div>
                    )}
                ...
                <div className="job-card-actions">
                  ...
                  {app.contract_status === 'signed' && (
                    <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '8px', padding: '10px 14px' }}>
                      <p style={{ fontSize: '13px', color: '#059669', fontWeight: '600', margin: '0 0 6px' }}>✓ Contract signed by client{app.contract_signed_at ? ` on ${new Date(app.contract_signed_at).toLocaleDateString()}` : ''}</p>
                      {app.contract_signed_by_name && <p style={{ fontSize: '12px', color: '#374151', margin: '0' }}>Signed by: {app.contract_signed_by_name}</p>}
                      {app.contract_signed_by_email && <p style={{ fontSize: '12px', color: '#374151', margin: '0' }}>{app.contract_signed_by_email}</p>}
                      {app.contract_signed_at && <p style={{ fontSize: '12px', color: '#6b7280', margin: '4px 0 6px' }}>{new Date(app.contract_signed_at).toLocaleString()}</p>}
                      {app.contract_file_path && (
                        <button style={{ background: 'none', border: 'none', color: '#667eea', cursor: 'pointer', fontSize: '13px', padding: 0, display: 'block' }}
                      ...
                      {app.contract_type === 'platform' && (
                        <button style={{ background: 'none', border: 'none', color: '#667eea', cursor: 'pointer', fontSize: '13px', padding: 0, display: 'block', marginTop: '4px' }}
                          onClick={async () => {
                            const res = await fetch('/api/generate-contract-pdf', {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify({ appId: app.id, userId: user.id })
                            })
                            ...
                            a.href = url
                            a.download = `PI-Connect-Agreement-${app.job?.title || 'Contract'}.pdf`
                            a.click()
                  ...
                  {/* Send invoice — show for both invoice and retainer modes after work starts */}
                    ...
                    <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '8px', padding: '12px 14px' }}>
                      ...
                      {app.payment_mode === 'escrow' && app.agreed_amount_cents && (
                        <p style={{ fontSize: '12px', color: '#166534', margin: '4px 0 0', fontWeight: '500' }}>
                          💰 Retainer amount: ${(app.agreed_amount_cents / 100).toFixed(2)} — contact PI Connect to arrange payout until Stripe Connect is enabled.
                        </p>
                      )}
                    ...
                    <>
                      ...
                      {app.deliverables_uploaded_at && (
                        <button type="button" className="btn-secondary-small"
                          ...
                          title="Generate a professional AI case summary to accompany your deliverables">
                          ✨ AI Case Summary
                        </button>
                      )}
                    </>
                  ...
                  {/* Additional funds request for retainer jobs */}
                      ...
                      app.cancel_requested_by ? (
                        ...
                        <button className="btn-danger-small"
                          ...
                          {app.payment_status === 'paid' && !app.work_started_at
                            ? 'Request Cancellation (retainer will be refunded if approved)'
          ...
          {jobs.filter(a => ['completed','cancelled','declined'].includes(a.status)).length > 0 && (
            <div style={{ marginTop: '24px' }}>
              <button
                ...
                <span>🗂 Archived Jobs ({jobs.filter(a => ['completed','cancelled','declined'].includes(a.status)).length})</span>
                <span>{showArchive ? '▲ Hide' : '▼ Show'}</span>
              </button>
              {showArchive && (
                <div style={{ marginTop: '12px' }}>
                  {jobs.filter(a => ['completed','cancelled','declined'].includes(a.status)).map(app => {
                    const job = app.job
                    const badge = getStatusBadge(app.status)
                          ...
                          <div>
                            ...
                            <p className="job-meta">
                              {app.completed_at ? `Completed ${new Date(app.completed_at).toLocaleDateString()}` :
                               app.cancelled_at ? (
                                 app.cancel_reason?.startsWith('Cancelled by client')
                                   ? `Cancelled by client · ${new Date(app.cancelled_at).toLocaleDateString()}`
                                   : `Cancelled ${new Date(app.cancelled_at).toLocaleDateString()}`
                               ) : ''}
                            </p>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                              {app.status === 'cancelled' && app.cancel_reason && (
                                <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '8px', padding: '10px 14px' }}>
                                  <p style={{ fontWeight: '600', fontSize: '13px', marginBottom: '2px', color: '#991b1b' }}>❌ Cancellation</p>
                                  <p style={{ fontSize: '13px', margin: 0, color: '#374151' }}>{app.cancel_reason}</p>
                                  {app.payment_status === 'refunded' && (
                                    <p style={{ fontSize: '12px', margin: '4px 0 0', color: '#6b7280' }}>Retainer was refunded to the client.</p>
                                  )}
                                </div>
                              )}
                              {app.payment_mode === 'escrow' && app.agreed_amount_cents && (
                                <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '10px 14px' }}>
                                  <p style={{ fontWeight: '600', fontSize: '13px', marginBottom: '2px' }}>💰 Retainer Payment</p>
                                  <p style={{ fontSize: '13px', margin: 0 }}>${(app.agreed_amount_cents / 100).toFixed(2)} — {app.payment_status === 'paid' ? 'funded by client' : app.payment_status === 'refunded' ? 'refunded to client' : app.payment_status}</p>
                                </div>
                              )}
                              {app.contract_status && (
                                <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '10px 14px' }}>
                                  ...
                                  {app.contract_status === 'signed' && (
                                    <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '6px', padding: '8px 12px', marginBottom: '8px' }}>
                                      <p style={{ fontSize: '12px', fontWeight: '600', color: '#166534', margin: '0 0 2px' }}>✓ Acceptance Record</p>
                                      {app.contract_signed_by_name && (
                                        <p style={{ fontSize: '12px', color: '#374151', margin: '0' }}>Signed by: {app.contract_signed_by_name}</p>
                                      )}
                                      {app.contract_signed_by_email && (
                                        <p style={{ fontSize: '12px', color: '#374151', margin: '0' }}>{app.contract_signed_by_email}</p>
                                      )}
                                      {app.contract_signed_at && (
                                        <p style={{ fontSize: '12px', color: '#6b7280', margin: '0' }}>{new Date(app.contract_signed_at).toLocaleString()}</p>
                                      )}
                                    </div>
                                  )}
                                  {app.contract_file_path && (
                                  ...
                                  {app.contract_type === 'platform' && app.contract_status === 'signed' && (
                                    <button style={{ background: 'none', border: 'none', color: '#667eea', cursor: 'pointer', fontSize: '13px', padding: 0, marginTop: '4px', display: 'block' }}
                                      onClick={async () => {
                                        const res = await fetch('/api/generate-contract-pdf', {
                                          method: 'POST',
                                          headers: { 'Content-Type': 'application/json' },
...
// ─── Main JobsPage ────────────────────────────────────────────────────────────
export default function JobsPage({ user, profile, onNavigate, initialTab, navSignal }) {
  ...
  // Only switch tabs on explicit navigation signal, not on every re-render
  useEffect(() => {
    if (initialTab && navSignal > 0) {
