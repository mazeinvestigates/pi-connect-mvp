...
const INVESTIGATION_TYPES = [
  'All Types', 'Surveillance', 'Background Investigation',
  'Fraud Investigation', 'Missing Person', 'Infidelity Investigation',
  'Corporate Investigation', 'Insurance Investigation'
]
...
function MarketplaceTab({ user, profile, onNavigate }) {
  ...
  const [filters, setFilters] = useState({ location: '', investigationType: '', urgency: '', budgetMin: '', budgetMax: '' })
  ...
  const loadJobs = async () => {
    try {
      ...
      if (filters.location) query = query.or(`city.ilike.%${filters.location}%,state.ilike.%${filters.location}%,location.ilike.%${filters.location}%`)
    ...
    <div>
      ...
      <div className="jobs-filters">
        <input type="text" placeholder="Location" value={filters.location}
          onChange={e => setFilters({ ...filters, location: e.target.value })} onBlur={loadJobs} />
      ...
      {profile?.type === 'pi' && recommendedJobs.length > 0 && !loading && (
        <RecommendedJobsSection jobs={recommendedJobs} onApply={handleApplyClick} />
      )}
        ...
        <div className="jobs-grid">
          {jobs.map(job => (
            <JobCard key={job.id} job={job}
              ...
              onSubcontract={profile?.type === 'pi' ? () => { setSelectedJob(job); setShowSubcontractModal(true) } : null}
      ...
      {showApplicationModal && selectedJob && (
        <JobApplicationModal job={selectedJob} user={user} piProfile={profile}
