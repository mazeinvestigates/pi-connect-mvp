...
export default function JobsPage({ user, profile, onNavigate }) {
  ...
  const loadJobs = async () => {
    try {
      ...
      // Calculate recommended jobs for PIs
      if (profile?.type === 'pi' && data && data.length > 0) {
        const piProfile = await loadPIProfile()
        if (piProfile) {
          const recommended = getRecommendedJobs(data, piProfile, 60) // 60% minimum match
          setRecommendedJobs(recommended.slice(0, 5)) // Top 5 recommendations
        }
      }
    } catch (error) {
  ...
  const loadPIProfile = async () => {
    try {
      ...
      console.error('Error loading PI profile:', error)
      return null
    }
  }
  ...
  return (
    <div className="jobs-page">
      ...
      {loading ? (
        ...
        <>
          {profile?.type === 'pi' && recommendedJobs.length > 0 && (
