            ...
            return (
              <div key={app.id} className="job-management-card" style={{ opacity: isPending ? 0.85 : 1, borderLeft: isPending ? '3px solid #fcd34d' : undefined }}>
                ...
                <div className="job-card-actions">
                  ...
                  {app.status === 'pending' && (
                    <button className="btn-danger-small"
                      ...
                      onClick={async () => {
                        ...
                        setUpdating(app.id)
                        await supabase.from('job_applications').update({ status: 'cancelled', cancelled_at: new Date().toISOString(), cancelled_by: user.id, cancel_reason: 'Withdrawn by PI' }).eq('id', app.id)
                        setUpdating(null)
                        loadMyJobs()
                      }}>
                  ...
                  {/* Cancellation */}
                  {app.status !== 'cancelled' && app.status !== 'completed' && app.status !== 'pending' && (
