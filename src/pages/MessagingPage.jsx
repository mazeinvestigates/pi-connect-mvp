...
export default function MessagingPage({ user, onNavigate }) {
  ...
  // Auto-open conversation if navigated from Message PI button
  useEffect(() => {
    const openId = sessionStorage.getItem('open_conversation_id')
    if (openId && conversations.length > 0) {
      const convo = conversations.find(c => c.id === openId)
      if (convo) {
        setSelectedConversation(convo)
        sessionStorage.removeItem('open_conversation_id')
      }
    }
  }, [conversations])
  ...
  const handleBackToList = async () => {
    // Mark all messages in current conversation as read before reloading
