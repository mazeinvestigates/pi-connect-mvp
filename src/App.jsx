...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Listen for auth changes — ignore token refreshes to prevent scroll reset on tab focus
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'TOKEN_REFRESHED') return // don't re-render on background token refresh
      setUser(session?.user ?? null)
      if (session?.user) {
        loadProfile(session.user.id)
      } else {
