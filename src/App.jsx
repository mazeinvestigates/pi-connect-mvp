...
// Memoized JobsPage — prevents remounting on parent re-renders (e.g. token refresh)
const MemoJobsPage = memo(JobsPage)
...
export default function App() {
  ...
  const setCurrentPageAndRef = (page) => {
  ...
  const [jobsNavSignal, setJobsNavSignal] = useState(0)
  ...
  useEffect(() => {
    ...
    if (connect === 'success') {
      sessionStorage.setItem('connect_success', '1')
      window.history.replaceState({}, '', window.location.pathname)
    } else if (connect === 'refresh') {
      sessionStorage.setItem('connect_refresh', '1')
      window.history.replaceState({}, '', window.location.pathname)
    }
  ...
  const loadProfile = async (userId) => {
    try {
      ...
      if (piProfile) {
        ...
        if (currentPageRef.current === 'home') setCurrentPageAndRef('dashboard')
  ...
  const navigateTo = (page) => {
    if (page === 'jobs:my-jobs') {
      ...
      setJobsNavSignal(s => s + 1)
      setCurrentPageAndRef('jobs')
    } else if (page === 'jobs') {
      setJobsInitialTab('marketplace')
      setJobsNavSignal(s => s + 1)
      setCurrentPageAndRef('jobs')
    } else {
      setCurrentPageAndRef(page)
    }
