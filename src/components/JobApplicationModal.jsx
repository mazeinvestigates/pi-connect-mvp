...
export default function JobApplicationModal({ job, user, piProfile, onClose, onSuccess, hasApplied, previousAppStatus }) {
  const canReApply = previousAppStatus === 'cancelled' && hasApplied
...
Return only the improved cover letter text, nothing else.`
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Delete any cancelled application first to satisfy the unique constraint
      if (existing?.status === 'cancelled') {
        const { error: deleteError } = await supabase
          ...
          .eq('status', 'cancelled')
        if (deleteError) {
          setError('Could not clear previous application. Please try again.')
          setLoading(false)
          return
        }
      }
    ...
    <div className="modal-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal application-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-header">
          <h2>{hasApplied && !canReApply ? 'Job Details' : 'Apply to Job'}</h2>
          <p>{job.title}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="modal-body">
          ...
          {canReApply && (
            <div className="alert" style={{ marginBottom: '16px', background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: '8px', padding: '10px 14px' }}>
              ⚠️ Your previous application was cancelled by the client. You can re-apply below.
            </div>
          )}
          ...
          <div className="modal-actions">
            <button type="button" className="btn-secondary" onClick={onClose}>
              {hasApplied && !canReApply ? 'Close' : 'Cancel'}
            </button>
            {(!hasApplied || canReApply) && (
              <button
                type="submit"
