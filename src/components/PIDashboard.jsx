          ...
          {acceptedJobs.map(app => (
            <div key={app.id} style={{ background: 'white', border: '1px solid #bbf7d0', borderRadius: '8px', padding: '12px 14px', marginBottom: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  {!app.payment_mode && (
                    <p style={{ fontSize: '12px', color: '#d97706', marginTop: '4px', fontWeight: '500' }}>
                      ⚠️ Action needed: Set payment mode in My Jobs tab
                    </p>
                  )}
                  {app.payment_mode && !app.work_started_at && app.payment_mode === 'invoice' && (
                    <p style={{ fontSize: '12px', color: '#2563eb', marginTop: '4px', fontWeight: '500' }}>
                      📄 Invoice mode — click Start Work when ready
                    </p>
                  )}
                  {app.payment_mode === 'escrow' && !app.work_started_at && (
                    <p style={{ fontSize: '12px', color: '#7c3aed', marginTop: '4px', fontWeight: '500' }}>
                      🔒 Escrow mode — awaiting upfront payment from client
                    </p>
                  )}
                ...
                <button className="btn-secondary-small" onClick={() => onNavigate('jobs')}>
                  Manage →
                </button>
              </div>
            </div>
