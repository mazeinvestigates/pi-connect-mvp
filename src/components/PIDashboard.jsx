...
export default function PIDashboard({ data, onRefresh, onNavigate }) {
  ...
  const handleConnectSetup = async () => {
    ...
    try {
      ...
      else alert(data.error || 'Could not start payout setup.')
    } catch (err) {
      alert('Could not start payout setup. Please try again.')
    } finally {
  ...
  const handleOpenStripePortal = async () => {
    try {
      const res = await fetch('/api/get-connect-portal', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: user.id })
      })
  ...
  return (
    <div className="dashboard-container">
      ...
      {connectStatus && !connectStatus.enabled && (
        <div style={{ background: connectStatus.connected ? '#fffbeb' : '#f0f9ff', border: `1px solid ${connectStatus.connected ? '#fcd34d' : '#bae6fd'}`, borderRadius: '10px', padding: '14px 18px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <p style={{ fontWeight: '600', fontSize: '14px', margin: '0 0 4px', color: connectStatus.connected ? '#92400e' : '#0c4a6e' }}>
              {connectStatus.connected ? '⚠️ Complete your payout setup' : '💳 Set up payouts to get paid'}
            </p>
            <p style={{ fontSize: '13px', color: '#6b7280', margin: 0 }}>
              {connectStatus.connected
                ? 'Your payout account needs additional information before you can receive payments.'
                : 'Connect your bank account to receive job payments directly. Takes about 5 minutes.'}
            </p>
          </div>
          <button onClick={handleConnectSetup} disabled={connectLoading}
            style={{ background: '#667eea', color: 'white', border: 'none', padding: '9px 18px', borderRadius: '8px', cursor: connectLoading ? 'not-allowed' : 'pointer', fontSize: '14px', fontWeight: '600', whiteSpace: 'nowrap', opacity: connectLoading ? 0.7 : 1 }}>
            {connectLoading ? 'Loading...' : connectStatus.connected ? 'Complete Setup' : 'Set Up Payouts'}
          </button>
        </div>
      )}

      {connectStatus?.enabled && (
        <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '10px', padding: '10px 18px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <span style={{ fontSize: '16px' }}>✓</span>
            <p style={{ fontSize: '13px', margin: 0, color: '#166534' }}>
              <strong>Payouts enabled</strong> — payments are automatically transferred to your connected bank account.
            </p>
          </div>
          <button onClick={handleOpenStripePortal}
            style={{ background: 'transparent', border: '1px solid #86efac', color: '#166534', padding: '5px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: '600', whiteSpace: 'nowrap' }}>
                ...
                <div>
                  ...
                  {app.payment_mode === 'escrow' && !app.work_started_at && app.payment_status === 'paid' && (
                    <p style={{ fontSize: '12px', color: '#059669', marginTop: '4px', fontWeight: '500' }}>
                      ✓ Retainer paid — go to My Jobs to begin work
                    </p>
                  )}
