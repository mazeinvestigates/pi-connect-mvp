...
import ReviewContractModal from './ReviewContractModal'

function DeliverablesList({ appId, onDownload }) {
  const [files, setFiles] = React.useState([])
  const [loading, setLoading] = React.useState(true)

  React.useEffect(() => {
    supabase.from('job_deliverables').select('*')
  ...
  return (
    <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '8px', padding: '12px 14px', marginBottom: '10px' }}>
      <p style={{ fontWeight: '600', fontSize: '13px', marginBottom: '8px', color: '#166534' }}>
        📁 Case files are ready for download
      </p>
      {files[0].notes && (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
        {files.map(f => (
          <button key={f.id} className="btn-secondary-small"
...
export default function ClientDashboard({ data, onRefresh, onNavigate }) {
  const { consultationRequests, postedJobs = [], user, profile } = data
  ...
  const toggleApps = (jobId) => setCollapsedApps(prev => ({ ...prev, [jobId]: !prev[jobId] }))
  ...
  const handleMessagePI = async (piUserId) => {
    try {
      ...
      if (conversation?.id) {
        sessionStorage.setItem('open_conversation_id', conversation.id)
      }
      onNavigate('messages')
    } catch (err) {
  ...
  const issueRefund = async (appId, jobId, piUserId, workStarted = false) => {
    try {
      // Find the transaction for this application
      ...
      if (!tx?.stripe_payment_intent_id) return // no payment to refund
      ...
      const res = await fetch('/api/refund-payment', {
        ...
        body: JSON.stringify({
          ...
          reason: workStarted ? 'Job cancelled after work started' : 'Job cancelled before work started',
          jobId,
          piUserId,
          workStarted
        })
      ...
      if (!workStarted) {
        // Update transaction status — transfer reversal handled server-side
  ...
  const handleDeclineApplication = async (appId) => {
    if (!window.confirm('Decline this application? The PI will be notified.')) return
    try {
  ...
  const handleDeleteJob = async (jobId, jobTitle, hasApplications) => {
    if (hasApplications) {
      alert('This job has applications and cannot be deleted. Cancel any active applications first, or wait until the job is completed.')
      return
    }
    if (!window.confirm(`Remove "${jobTitle}"? This cannot be undone.`)) return
    try {
      const { error } = await supabase
        .from('jobs')
  ...
  const handleCancelJob = async (appId, jobId, wasRetainerPaid, piUserId, workStarted = false) => {
    const reason = window.prompt('Please provide a reason for cancellation:')
    if (!reason) return

    let confirmMsg
    if (wasRetainerPaid && !workStarted) {
      confirmMsg = 'Cancel this job? The retainer will be reversed from the PI and a full refund issued to your account.'
    } else if (wasRetainerPaid && workStarted) {
      confirmMsg = 'Cancel this job? Work has already started. The PI will need to issue any refund directly. Contact the PI to discuss.'
    } else {
    ...
    try {
      if (wasRetainerPaid) {
        const refundResult = await issueRefund(appId, jobId, piUserId, workStarted)
        if (refundResult?.status === 'manual_required') {
          alert('The job has been cancelled. Since work had already started, the PI will need to issue any refund directly from their account. Please contact the PI to arrange.')
        }
      }
      await supabase.from('job_applications').update({
        ...
        cancel_reason: `Cancelled by client: ${reason}`,
        payment_status: wasRetainerPaid && !workStarted ? 'refunded' : undefined
      }).eq('id', appId)
      await supabase.from('jobs').update({ status: 'open' }).eq('id', jobId)
      onRefresh()
      if (wasRetainerPaid && !workStarted) {
        alert('Job cancelled. The retainer has been reversed and a full refund issued to your payment method.')
      } else if (!wasRetainerPaid) {
        alert('Job cancelled.')
      }
    } catch (err) {
  ...
  const handleDownloadDeliverable = async (filePath, fileName) => {
  ...
  const handleCompleteJob = async (appId, jobId) => {
    if (!window.confirm('Mark this job as complete? This confirms you have received the deliverables and the job is finished.')) return
    try {
      await supabase.from('job_applications').update({
        status: 'completed',
      ...
      // Mark any referral fee as earned now that job is complete and paid
      await supabase.from('referrals')
        .update({
          fee_status: 'earned',
          fee_earned_at: new Date().toISOString()
        })
        .eq('job_id', jobId)
        .eq('status', 'accepted')
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-section">
        ...
        {postedJobs.length === 0 ? (
          ...
          const renderJob = (job) => {
              ...
              const acceptedApp = apps.find(a => ['accepted','in_progress','completed'].includes(a.status))
                ...
                <div key={job.id} className="job-management-card">
                  <div className="job-card-header">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      {job.status === 'open' && apps.filter(a => !['cancelled','declined'].includes(a.status)).length === 0 && (
                        <button
                          type="button"
                          onClick={() => handleDeleteJob(job.id, job.title, apps.filter(a => !['cancelled','declined'].includes(a.status)).length > 0)}
                          style={{ background: 'none', border: '1px solid #fca5a5', color: '#ef4444', padding: '3px 10px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: '600' }}>
                          Remove
                        </button>
                      )}
                    </div>
                  ...
                  {apps.length === 0 ? (
                    ...
                    <div className="applications-section">
                      <button
                        type="button"
                        onClick={() => toggleApps(job.id)}
                        style={{ background: 'none', border: 'none', padding: '0 0 8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', width: '100%', textAlign: 'left' }}>
                        <h4 style={{ fontSize: '14px', fontWeight: '600', color: '#374151', margin: 0 }}>
                          Applications ({apps.length})
                        </h4>
                        <span style={{ fontSize: '12px', color: '#9ca3af', marginLeft: 'auto' }}>
                          {collapsedApps[job.id] ? '▼ Show' : '▲ Hide'}
                        </span>
                      </button>
                      {!collapsedApps[job.id] && apps.map(app => {
                          ...
                          <div key={app.id} className="application-item" style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                              <div>
                                ...
                                <p style={{ fontSize: '13px', color: '#6b7280' }}>
                                  📍 {pi?.city}, {pi?.state}
                                  {app.proposed_rate && ` · $${app.proposed_rate}${app.rate_type === 'flat' ? ' flat fee' : '/hr'}`}
                            ...
                            {app.cancel_requested_by && app.cancel_requested_by !== user.id && app.status !== 'cancelled' && (
                              <div style={{ background: '#fef3c7', border: '1px solid #fcd34d', borderRadius: '8px', padding: '10px 14px', marginBottom: '8px', fontSize: '13px' }}>
                                ...
                                <button className="btn-primary" onClick={() => handleApproveCancellation(app.id, job.id, app.payment_status === 'paid' && app.payment_mode === 'escrow')}>
                            ...
                            {app.deliverables_uploaded_at && (
                              <DeliverablesList appId={app.id} onDownload={handleDownloadDeliverable} />
                            )}

                            {/* Contract status */}
                            {(app.status === 'accepted' || app.status === 'in_progress') && app.contract_status === 'pending_signature' && (
                              <div style={{ background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: '8px', padding: '10px 14px', marginBottom: '8px' }}>
                                <p style={{ fontWeight: '600', fontSize: '13px', marginBottom: '6px' }}>📄 PI has sent a contract for your review</p>
                                <button className="btn-primary"
                                  onClick={() => { setContractApp(app); setContractJob(job); setShowContractModal(true) }}>
                                  Review & Sign Contract
                                </button>
                              </div>
                            )}
                            {app.contract_status === 'signed' && (
                              <p style={{ fontSize: '13px', color: '#059669', fontWeight: '500', marginBottom: '8px' }}>✓ Contract signed</p>
                            )}
                            {app.contract_status === 'declined' && (
                              <p style={{ fontSize: '13px', color: '#dc2626', fontWeight: '500', marginBottom: '8px' }}>✗ Contract declined — reason: {app.contract_decline_reason}</p>
                            )}
                            ...
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                              {app.status === 'pending' && (
                                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                  <button
                                    style={{ padding: '8px 20px', fontSize: '14px', fontWeight: '600', background: '#667eea', color: 'white', border: 'none', borderRadius: '8px', cursor: accepting === app.id ? 'not-allowed' : 'pointer', opacity: accepting === app.id ? 0.7 : 1 }}
                                  ...
                                  <button
                                    style={{ padding: '8px 20px', fontSize: '14px', fontWeight: '600', background: 'white', color: '#dc2626', border: '1px solid #fca5a5', borderRadius: '8px', cursor: 'pointer' }}
                              ...
                              {/* Escrow: prompt client to pay retainer */}
                              {(app.status === 'accepted' || app.status === 'in_progress') && app.payment_mode === 'escrow' && !app.work_started_at && (
                                <>
                                  {app.payment_status === 'paid' ? (
                                    <span style={{ fontSize: '13px', color: '#059669', fontWeight: '600' }}>✓ Retainer paid — work can begin</span>
                                  ) : app.escrow_amount_cents ? (
                                    <button className="btn-primary" onClick={() => handlePayment(job, app)}>
                                      🔒 Pay Retainer — ${(app.escrow_amount_cents / 100).toFixed(2)}
                                    </button>
                                  ) : (
                                    <span style={{ fontSize: '13px', color: '#d97706' }}>⏳ PI is setting the retainer amount</span>
                                  )}
                                  <p style={{ fontSize: '12px', color: '#6b7280', width: '100%', marginTop: '2px' }}>
                                    PI requires retainer payment before starting work.
                                  </p>
                                </>
                              )}

                              {/* Invoice: pay when PI sends invoice */}
                              {(app.status === 'accepted' || app.status === 'in_progress' || app.status === 'completed') && app.invoice_sent_at && (
                                <>
                                  ...
                                  {app.payment_mode === 'escrow' ? (
                                    // Escrow already funded — invoice is just an itemization
                                    <span style={{ fontSize: '13px', color: '#059669', fontWeight: '500' }}>
                                      ✓ Work summary received from PI
                                    </span>
                                  ) : app.payment_status === 'paid' ? (
                                    <span style={{ fontSize: '13px', color: '#059669', fontWeight: '600' }}>✓ Invoice paid</span>
                                  ) : (
                                    <button className="btn-primary" onClick={() => handlePayment(job, app)}>
                              ...
                              {app.status === 'accepted' && !app.work_started_at && !app.cancel_requested_by && (
                                <button className="btn-danger-small"
                                  onClick={() => handleCancelJob(app.id, job.id, app.payment_status === 'paid' && app.payment_mode === 'escrow', app.applicant_id, !!app.work_started_at)}>
                                  {app.payment_status === 'paid' && app.payment_mode === 'escrow'
                                    ? 'Cancel Job (refund retainer)'
                                    : 'Cancel Job'}
                                </button>
                              )}
                              {app.status === 'accepted' && app.work_started_at && !app.cancel_requested_by && (
                                <span style={{ fontSize: '12px', color: '#9ca3af' }}>Work in progress — cancellation requires PI agreement</span>
                              )}

                              {/* Mark as Complete */}
                              {(app.status === 'accepted' || app.status === 'in_progress') &&
                               app.deliverables_uploaded_at &&
                               (app.payment_mode === 'escrow' || app.payment_status === 'paid') && (
                                <button className="btn-primary"
                                  style={{ background: '#059669' }}
                                  onClick={() => handleCompleteJob(app.id, job.id)}>
                                  ✓ Mark Job as Complete
                                </button>
                              )}

                              {app.status === 'completed' && (
                                <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '8px', padding: '10px 14px', width: '100%' }}>
                                  <p style={{ fontWeight: '600', fontSize: '13px', color: '#166534', margin: 0 }}>
                                    ✓ Job completed — {app.completed_at ? new Date(app.completed_at).toLocaleDateString() : ''}
                                  </p>
                                </div>
                              )}
                            </div>
            ...
            const activeJobs = postedJobs.filter(j => !['completed','cancelled','closed'].includes(j.status))
            const archivedJobs = postedJobs.filter(j => ['completed','cancelled','closed'].includes(j.status))
            return (
              <>
                {activeJobs.length === 0 && archivedJobs.length > 0 && (
                  <p style={{ fontSize: '13px', color: '#6b7280', marginBottom: '12px' }}>All your jobs are in the archive.</p>
                )}
                  ...
                  <div style={{ marginTop: '24px' }}>
                    ...
                    {showArchive && (
                      <div style={{ marginTop: '12px' }}>
                        {archivedJobs.map(job => renderJob(job))}
                      </div>
                    )}
                  </div>
                )}
      ...
      {showContractModal && contractApp && contractJob && (
        <ReviewContractModal
          app={contractApp}
          job={contractJob}
          user={user}
          onClose={() => { setShowContractModal(false); setContractApp(null); setContractJob(null) }}
          onSuccess={() => { setShowContractModal(false); setContractApp(null); setContractJob(null); onRefresh(); alert('Contract signed successfully!') }}
          onDecline={() => { setShowContractModal(false); setContractApp(null); setContractJob(null); onRefresh(); alert('Contract declined. The job has been returned to open status.') }}
        />
      )}
