...
export default function ClientDashboard({ data, onRefresh, onNavigate }) {
  ...
  const handleCancelJob = async (appId, jobId, wasEscrowPaid) => {
    const reason = window.prompt('Please provide a reason for cancellation:')
    if (!reason) return
    const confirmMsg = wasEscrowPaid
      ? 'Cancel this job? Since escrow was funded, a full refund will be issued to your account.'
      : 'Are you sure you want to cancel this job?'
    if (!window.confirm(confirmMsg)) return
    try {
      if (wasEscrowPaid) {
        await issueRefund(appId)
      }
      await supabase.from('job_applications').update({
        ...
        payment_status: wasEscrowPaid ? 'refunded' : null
      }).eq('id', appId)
      await supabase.from('jobs').update({ status: 'open' }).eq('id', jobId)
      onRefresh()
      if (wasEscrowPaid) alert('Job cancelled. A full refund has been issued to your payment method.')
    } catch (err) {
                            ...
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                              ...
                              {(app.status === 'accepted' || app.status === 'in_progress') && app.payment_mode === 'escrow' && !app.work_started_at && (
                                <>
                                  ...
                                  ) : app.escrow_amount_cents_cents ? (
                                    <button className="btn-primary" onClick={() => handlePayment(job, app)}>
                                      🔒 Pay Retainer — ${(app.escrow_amount_cents_cents / 100).toFixed(2)}
                                    </button>
                                  ) : (
                              ...
                              {app.status === 'accepted' && !app.work_started_at && !app.cancel_requested_by && (
                                <button className="btn-danger-small"
                                  onClick={() => handleCancelJob(app.id, job.id, app.payment_status === 'paid' && app.payment_mode === 'escrow')}>
