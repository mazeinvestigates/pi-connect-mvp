...
import { sendPaymentConfirmationEmails } from '../emailUtils'
...
function StripeCardForm({ breakdown, job, application, user, invoiceStyle, showPlatformFee, expenses, onSuccess, onError, onBack }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create PaymentIntent via serverless function (secret key stays server-side)
      const piResponse = await fetch('/api/create-payment-intent', {
        ...
        body: JSON.stringify({
          ...
          jobId: job?.id || '',
          piId: application?.applicant_id || '',
          clientId: user.id,
          laborCents: breakdown.laborCents,
          expensesCents: breakdown.expensesCents || 0,
      ...
      if (paymentIntent.status === 'succeeded') {
        const { data: transaction, error: txError } = await supabase
          ...
          .insert({
            client_id: user.id,
            pi_id: application.applicant_id || null,
            job_id: job.id,
        ...
        // Send payment confirmation emails to both parties (non-blocking)
        sendPaymentConfirmationEmails({
          clientId: user.id,
          clientEmail: user.email,
          clientName: user.email,
          piId: application.applicant_id,
          piEmail: null, // fetched by edge function if needed
          piName: 'PI',
          jobTitle: job.title,
          clientAmount: (breakdown.totalCents / 100).toFixed(2),
          piAmount: (breakdown.piTotalPayoutCents / 100).toFixed(2)
...
// ─── Main modal ──────────────────────────────────────────────────────────────
export default function PaymentModal({ job, application, user, profile, onClose, onSuccess }) {
  // Determine if this is a client paying (simple flow) or PI setting up payment
  const isClientPaying = !profile || profile.type === 'client' || profile.role === 'client'

  // Pre-fill amount from invoice/escrow data
  const getInitialAmount = () => {
    if (application?._isAdditionalPayment && application?.additional_escrow_requested_cents) {
      return String(application.additional_escrow_requested_cents / 100)
    }
    if (application?.invoice_amount_cents) {
      return String(application.invoice_amount_cents / 100)
    }
    if (application?.escrow_amount_cents_cents) {
      return String(application.escrow_amount_cents_cents / 100)
    }
    return application?.proposed_rate ? String(application.proposed_rate) : ''
  }

  // Client starts directly on 'pay', PI starts on 'setup'
  const [step, setStep] = useState(isClientPaying ? 'pay' : 'setup')
  const [laborAmount, setLaborAmount] = useState(getInitialAmount)
      ...
      <div className="modal-overlay">
        <div className="modal payment-modal" onClick={e => e.stopPropagation()}>
          <div className="payment-success">
            ...
            <p style={{ fontSize: '13px', color: '#6b7280', marginTop: '12px' }}>Redirecting...</p>
          </div>
        </div>
      </div>
    )
  ...
  // Card entry
  if (step === 'pay') {
    const totalAmount = parseFloat(laborAmount) || 0
    return (
      <div className="modal-overlay" onMouseDown={e => { if (e.target === e.currentTarget) onClose() }}>
        <div className="modal payment-modal" onMouseDown={e => e.stopPropagation()}>
          ...
          <div className="modal-body">
            ...
            {isClientPaying ? (
              // Simple client payment summary
              <div className="info-box" style={{ marginBottom: '20px' }}>
                <p style={{ fontWeight: '600', marginBottom: '12px', fontSize: '15px' }}>Payment Summary</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span>{job.title}</span>
                    <span>${totalAmount.toFixed(2)}</span>
                  </div>
                  {application?.invoice_notes && (
                    <p style={{ fontSize: '13px', color: '#6b7280', fontStyle: 'italic', borderTop: '1px solid #e5e7eb', paddingTop: '8px' }}>
                      Note from PI: {application.invoice_notes}
                    </p>
                  )}
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #e5e7eb', paddingTop: '8px', fontWeight: '700', fontSize: '16px' }}>
                    <span>Total Due</span>
                    <span>${totalAmount.toFixed(2)}</span>
                  </div>
                </div>
            ...
            <Elements stripe={stripePromise}>
              <StripeCardForm
                ...
                onError={setError}
                onBack={isClientPaying ? null : () => setStep('setup')}
              />
            </Elements>
          </div>
        </div>
