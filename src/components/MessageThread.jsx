...
import { sendNewMessageEmail } from '../emailUtils'
...
export default function MessageThread({ conversation, currentUser, onBack }) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const recipientEmail = conversation.user1_id === currentUser.id
        ? conversation.user2_email
        : conversation.user1_email
      
      const senderName = conversation.user1_id === currentUser.id
        ? conversation.user1_name
        : conversation.user2_name

      // Queue email (will only send if user has notifications enabled)
      await sendNewMessageEmail(
        recipientId,
        recipientEmail,
        senderName || 'Someone',
        filterResult.filtered.substring(0, 150)
      )
