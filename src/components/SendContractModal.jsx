...
export function generateContractHTML({ piName, clientName, jobTitle, jobLocation, rate, rateType, escrowAmount, invoiceAmount, paymentMode, today, includeSubcontractorDisclosure }) {
  return `
    <div style="font-family: Georgia, serif; max-width: 700px; margin: 0 auto; padding: 40px; color: #1a1a1a; line-height: 1.7;">
      ...
      <p>${
        paymentMode === 'escrow' && escrowAmount
          ...
          : rate
          ? `Client agrees to compensate PI at the rate of <strong>$${rate}${rateType === 'flat' ? ' (flat fee)' : '/hour'}</strong>, plus documented expenses. A 10% platform fee applies to labor charges.`
      ...
      ${includeSubcontractorDisclosure ? '<h3 style="margin-top: 24px; margin-bottom: 8px; font-size: 15px;">Use of Subcontractors</h3><p>PI reserves the right to engage licensed and qualified subcontractors to assist in fulfilling this engagement. Any subcontractor engaged by PI shall hold all required licenses and shall be bound by the same confidentiality and legal compliance obligations set forth in this Agreement. PI remains solely responsible to Client for the quality and delivery of all work product, regardless of whether a subcontractor is used. Client identity and contact information shall not be disclosed to any subcontractor.</p>' : ''}
...
export default function SendContractModal({ app, job, user, piProfile, onClose, onSuccess }) {
  ...
  const [includeSubcontractorDisclosure, setIncludeSubcontractorDisclosure] = useState(false)

  const today = new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })

  const contractHTML = useMemo(() => generateContractHTML({
    ...
    rateType: app?.rate_type || 'hourly',
    escrowAmount: app?.escrow_amount_cents_cents ? (app.escrow_amount_cents_cents / 100).toFixed(2) : null,
    invoiceAmount: app?.invoice_amount_cents ? (app.invoice_amount_cents / 100).toFixed(2) : null,
    paymentMode: app?.payment_mode || null,
    today,
    includeSubcontractorDisclosure
  }), [includeSubcontractorDisclosure, app, job, piProfile, today])
        ...
        <div className="modal-body">
          ...
          {mode === 'platform' && (
            <div>
              ...
              <div style={{ marginBottom: '14px', padding: '10px 14px', background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: '8px' }}>
                <label style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', cursor: 'pointer' }}>
                  <input type="checkbox" checked={includeSubcontractorDisclosure}
                    onChange={e => setIncludeSubcontractorDisclosure(e.target.checked)}
                    style={{ marginTop: '3px', flexShrink: 0 }} />
                  <div>
                    <p style={{ fontWeight: '600', fontSize: '13px', margin: '0 0 2px', color: '#92400e' }}>Include subcontractor disclosure</p>
                    <p style={{ fontSize: '12px', color: '#78350f', margin: 0 }}>
                      Check this if you plan to use a licensed subcontractor. Discloses that a sub may assist while keeping client identity confidential from them.
                    </p>
                  </div>
                </label>
              </div>
