...
import { applyPendingReferralToProfile } from '../referralUtils'
  ...
  const saveStep5 = async () => {
    ...
    try {
      const { error } = await supabase.from('pi_profiles').update({
        ...
        bio: formData.bio,
        notification_radius_miles: formData.notificationRadius,
      ...
      // Apply any pending referral code to the profile
      await applyPendingReferralToProfile(user.id)
                ...
                <div className="form-group" style={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <div className="form-group" style={{ marginTop: '20px' }}>
                    ...
                    <p style={{ fontSize: '12px', color: '#6b7280', marginBottom: '6px' }}>
                      How far are you willing to travel for a job? This controls which searches you appear in.
                    </p>
                    <input type="range" min="0" max="5" step="1"
                      value={[25, 50, 100, 250, 500, -1].indexOf(formData.serviceRadius) !== -1
                        ? [25, 50, 100, 250, 500, -1].indexOf(formData.serviceRadius) : 2}
                      onChange={e => {
                        const values = [25, 50, 100, 250, 500, -1]
                        update('serviceRadius', values[parseInt(e.target.value)])
                      }}
                      style={{ width: '100%' }} />
                    <p style={{ fontSize: '12px', color: '#374151', marginTop: '4px', marginBottom: '16px' }}>
                    ...
                    <div style={{ padding: '8px 0' }}>
                      <input
                        ...
                        value={[25, 50, 100, 250, 500, -1].indexOf(formData.notificationRadius) !== -1
                          ? [25, 50, 100, 250, 500, -1].indexOf(formData.notificationRadius) : 2}
                        onChange={e => {
                          const values = [25, 50, 100, 250, 500, -1]
                          update('notificationRadius', values[parseInt(e.target.value)])
                        }}
                        style={{ width: '100%' }}
                      />
                    ...
                    <small>
                      {formData.notificationRadius === -1
                        ? 'Receive job match emails for all jobs nationwide'
                        : `Receive job match emails for jobs within ${formData.notificationRadius} miles of your location`}
                    </small>
                  </div>
                </div>
              </div>
        ...
        {step === 6 && (
          <div className="onboarding-step onboarding-done">
            ...
            <div className="done-actions">
              <button className="btn-onboarding-primary" onClick={() => onComplete('subscription')}>
                Go to My Dashboard
              </button>
